...
namespace XManager.Forms
{
    public partial class BurnMedia : Form
    {
        ...
        public BurnMedia(string rawPath, string dicomPath)
        {
            InitializeComponent();
            this._rawPath = rawPath;
            this._dicomPath = dicomPath;
        }

        private void BurnMedia_Load(object sender, EventArgs e)
        {
            cbxType.Items.Add("raw");
            cbxType.Items.Add("bmp");
            cbxType.Items.Add("Dicom");
            cbxType.SelectedIndex = 0;

            DirectoryInfo di = new DirectoryInfo(this._rawPath);
            if(di.Exists)
            {
                foreach (var item in di.GetFiles())
                {
                    ...
                    string onlyFileName = name.Substring(0, index);
                    checkedListBox.Items.Add(onlyFileName);
                }
            }
            //
            ...
            try
            {
                ...
                foreach (string uniqueRecorderId in discMaster)
                {
                    ...
                    cbxdevices.Items.Add(discRecorder2);
                }
            ...
            tbxVolumeLabel.Text = now.Year + "_" + now.Month + "_" + now.Day;
        ...
        private void BurnMedia_FormClosing(object sender, FormClosingEventArgs e)
        {
            //
            // Release the disc recorder items
            //
            foreach (MsftDiscRecorder2 discRecorder2 in cbxdevices.Items)
            {
        ...
        private void cbxdevices_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            var discRecorder =
                (IDiscRecorder2)cbxdevices.Items[cbxdevices.SelectedIndex];
        ...
        /// <param name="sender"></param>
        /// <param name="e"></param>
        private void cbxdevices_Format(object sender, ListControlConvertEventArgs e)
        {
        ...
        private void buttonBurn_Click(object sender, EventArgs e)
        {
            ...
            if(checkCount ==0)
            {
                MessageBox.Show("There is no Select File.");
                return;
            }
            if (_isBurning)
            {
            ...
            else
            {
                ...
                MakeFile(cbxType.SelectedItem as string);
        ...
        {
            // backgroundBurnWorker.ReportProgress(50);
        ...
        private bool CreateMediaFileSystem(IDiscRecorder2 discRecorder, object[] multisessionInterfaces, out IStream dataStream)
        {
            ...
            try
            {
                ...
                fileSystemImage.VolumeName = tbxVolumeLabel.Text;
        ...
        private void MakeFile(string type)
        {
            try
            {
                UpdateCapacity();
        ...
        private int GetRawDataCount(string path)
        {
            int nCount = 0;
        ...
        private List<string> GetPathNName(string path)
        {
            List<string> pathNName = new List<string>();
            DirectoryInfo di = new DirectoryInfo(path);
            if (di.Exists == true)
            {
                foreach (var item in di.GetFiles())
                {
                    pathNName.Add(Path.Combine(path, item.FullName));
                }
            }
        ...
        private void btnSaveUSB_Click(object sender, EventArgs e)
        {
            ...
            {
                string strFullPathFile = dlg.SelectedPath;
                string message = "Do you want to save to the path below?" + "\n" + "Path : " + strFullPathFile;
                if (MessageBox.Show(message, "",MessageBoxButtons.YesNo) == DialogResult.Yes)
                {
                    SaveFiles(type, strFullPathFile);
        ...
        private void SaveFiles(string type,string saveFolderPath)
        {
        }

        private void FileCopy(string orgFilePath, string newFilePath)
        {
            FileInfo di = new FileInfo(newFilePath);
            if(di.Exists == true)
            {
                string message = "The file exists in the location you want to save it.\n Do you want to overwrite the file?";
                DialogResult dlg = MessageBox.Show(message, "", MessageBoxButtons.YesNo);
