...
namespace XManager.Forms
{
    public partial class ProfileListForm : Form
    {
        ...
        private void ControlAdd()
        {
            ...
            _histogramControl.SetHistogramParam(CStatus.Instance().GetDrawBox().ImageManager.GetProfileHistogramParamList());
        ...
        public void ProfileDataGridClearReUpdate()
        {
            try
            {
                ...
                int index = 0;
                _selectIndex = -1;
                foreach (ProfileFigure profile in CStatus.Instance().GetDrawBox().TrackerManager.GetProfileAllList())
                {
                    index++;
                    ...
                    string distance = CStatus.Instance().GetDrawBox().ImageManager.DistancePointToPoint(orgStartPoint, orgEndPoint).ToString();
        ...
        private void ProfileForm_FormClosed(object sender, FormClosedEventArgs e)
        {
            if (CloseEventDelegate != null)
                CloseEventDelegate();
            CStatus.Instance().ProfileWidth = 1;
            CStatus.Instance().DerivativeType = eDerivativeType.None;
        }
        ...
        {
            tProfileResult result = (tProfileResult)CStatus.Instance().GetDrawBox().TrackerManager.GetSelectedProfile().GetResult();
            PointF orgStartPoint = CStatus.Instance().GetDrawBox().ImageManager.GetOrgPoint(result.StartPoint);
            PointF orgEndPoint = CStatus.Instance().GetDrawBox().ImageManager.GetOrgPoint(result.EndPoint);
        ...
        public void HistogramGraphReNewal()
        {
            try
            {
                if (_histogramControl != null)
                {
                    _histogramControl.ImageBit = CStatus.Instance().GetDrawBox().ImageManager.GetBit();
                    _histogramControl.IsColor = CStatus.Instance().GetDrawBox().ImageManager.IsColor();
                    _histogramControl.HistogramGraphReNewal(CStatus.Instance().GetDrawBox().ImageManager.GetProfileHistogramParamList());
                }
            }
            catch (Exception err)
            {
        ...
        public void MoveEventUpdate(List<HistogramParams> param, PointF orgStartPt, PointF orgEndPt)
        {
            ...
            CStatus.Instance().IsDrawingProfile = true;
            th.Start();
        }
        ...
        public void MoveEventThread(List<HistogramParams> histgramParam, PointF orgStartPt, PointF orgEndPt)
        {
            try
            {
                ...
                CStatus.Instance().IsDrawingProfile = false;
                //MarkReset();
            }
            catch (Exception err)
            {
        ...
        private void nupdnWidth_ValueChanged(object sender, EventArgs e)
        {
            ...
            if (value <= 0)
                return;
            CStatus.Instance().ProfileWidth = Convert.ToInt32(value);
            HistogramGraphReNewal();
            CStatus.Instance().GetDrawBox().ReUpdate();
        }
        ...
        private void cbxDerivativeType_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            if (index == 0)
                CStatus.Instance().DerivativeType = eDerivativeType.None;
            if (index == 1)
                CStatus.Instance().DerivativeType = eDerivativeType.OneDerivative;
            if (index == 2)
                CStatus.Instance().DerivativeType = eDerivativeType.TwoDerivative;
