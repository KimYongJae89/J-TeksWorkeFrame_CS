...
namespace XManager.Controls
{
    public partial class LevelingControl : UserControl
    {
        ...
        private void HistogramControl_Load(object sender, EventArgs e)
        {
            ...
            _histogramPanel.SetHistogramParam(CStatus.Instance().GetDrawBox().ImageManager.GetSelectedHistogramParam());
            ...
            _slider.SetSliderMinMax(CStatus.Instance().Settings.HistogramMin, CStatus.Instance().Settings.HistogramMax);
        ...
        {
            if (CStatus.Instance().GetDrawBox().ImageManager.DisPlayImage.Depth == KiyLib.DIP.KDepthType.Dt_8)
            {
                nupdnMin.Minimum = nupdnMax.Minimum = 0;
                nupdnMin.Maximum = nupdnMax.Maximum = 255;
            }
            else if (CStatus.Instance().GetDrawBox().ImageManager.DisPlayImage.Depth == KiyLib.DIP.KDepthType.Dt_16)
            {
                nupdnMin.Minimum = nupdnMax.Minimum = 0;
            ...
            int min = CStatus.Instance().Settings.HistogramMin;
            int max = CStatus.Instance().Settings.HistogramMax;
        ...
        public void EditParam(ref IpGreyLevelingParams param)
        {
