...
namespace ImageManipulator.Data
{
    ...
    public class ProfileFigure : Figure //  : LineFigure
    {
        ...
        private void GetCalcPointListOfLine(PointF startPoint, PointF endPoint, bool isInitialize = false)
        {
            ...
            float width = Status.Instance().SelectedViewer.ImageManager.DisPlayImage.Width - 1;
            float height = Status.Instance().SelectedViewer.ImageManager.DisPlayImage.Height - 1;
        ...
        public override void Draw(Graphics g)
        {
            try
            {
                ...
                if (this.Selectable)
                {
                    Status.Instance().LogManager.AddLogMessage("Select Profile", "Start : " + DrawLineStartPoint.ToString() + " End :" + DrawLineEndPoint.ToString());
        ...
        private void DrawProfileLine(Graphics g)
        {
            ...
            if (this.Selectable && Status.Instance().ProfileWidth != 1)
            {
                ...
                List<Point> realPts = KLineProfile.GetProjectionRectPoints((int)orgStartPt.X, (int)orgStartPt.Y, (int)orgEndPt.X, (int)orgEndPt.Y, Status.Instance().ProfileWidth);
        ...
        private void DrawMarkGrid(Graphics g)
        {
            try
            {
                ...
                string length = Status.Instance().GetDistance(orgStartPt, orgEndPt);

                string labelText = length;
                Brush brush = Brushes.Black;
                Font font = new Font("고딕", 7);
                int halfTrackRectWidth = TrackRectWidth / 2;
