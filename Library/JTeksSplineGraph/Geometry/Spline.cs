...
namespace JTeksSplineGraph.Geometry
{
    ...
    public class Spline
    {
        ...
        private ControlSize ownControlSize = null;
        ...
        {
            if(ownControlSize == null)
            {
                ownControlSize = new ControlSize(size);
            }
        }
        ...
        public Point[] SplinePoint
        {
            get
            {
                ...
                for (int i = 0; i < pts.Length; i++)
                {
                    ...
                    CalcMaxMinLimitPoint(ref pts[i]);
                }
                return pts;
            }
        }
        ...
        public PointF[] SplinePointF
        {
            get
            {
                ...
                for (int i = 0; i < pts.Length; i++)
                {
                    ...
                    CalcMaxMinLimitPointF(ref pts[i]);
                }
                return pts;
            }
        }
        ...
        private void CalcMaxMinLimitPoint(ref Point point)
        {
            if (ownControlSize == null)
                return;
            if (point.X <= 0)
            ...
            if (point.Y <= 0)
                point.Y = 0;
            if (point.X >= ownControlSize.Width - 1)
                point.X = ownControlSize.Width - 1;
            if (point.Y >= ownControlSize.Height - 1)
                point.Y = ownControlSize.Height - 1;
        }

        private void CalcMaxMinLimitPointF(ref PointF point)
        {
            if (ownControlSize == null)
                return;
            if (point.X <= 0)
