...
{
    /*public class ClockwiseComparerPoint : IComparer<Point>
    {
        public int Compare(Point pt1, Point pt2)
        {
            if (pt1.X >= 0)
            {
                ...
                return -Comparer<float>.Default.Compare(pt1.Y, pt2.Y);
            }
            else
            {
                if (pt2.X >= 0)
    ...
    public class ClockwiseComparerPointF : IComparer<PointF>
    {
    ...
    /// <summary>
    /// "string을 숫자로 가정하여 비교한다."
    /// 예를들어 특정 폴더의 파일명들이 숫자일때, 
    /// 정렬하면 숫자를 문자열로 취급해 1 다음에 10이 오는등 제대로 정렬되지 않는다.
    /// 하지만 StringAsNumericComparer를 사용하면 제대로 정렬할수 있다.
    /// 사용예) Array.Sort(filePathArray, new NumericComparer());
    /// </summary>
    public class StringAsNumericComparer : IComparer
    {
